...
/**
  ...
  */
public class BaselinePageMethod extends RetrievalMethod {
    ...
    private final Logger logger = LoggerFactory.getLogger(BaselinePageMethod.class);

    private final SimType similarity;
    /**
     * @param args Arguments parsed from the configuration file.
     */
    public BaselinePageMethod(PrototypeMain.PrototypeArgs args, QueryRanker.ContentType cType, SimType simType) {
        super(args, cType);
    ...
    public String getName() {
        return "baseline_page_" + this.similarity;
    }
    ...
    @Override
    public QueryType getQueryType() {return QueryType.ARTICLE;}
    ...
    public Map<String, String> parseQueries(boolean train) {
        ...
        logger.info("Parsing page " + ((train) ? "training" : "test") + " queries.");
        ...
        if(qrelStream != null) {
            for(Data.Page page : DeserializeData.iterableAnnotations(qrelStream)) {
                queries.put(page.getPageId(), page.getPageName());
            }
        }
